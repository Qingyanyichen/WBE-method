...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'IN':
        mat_data = sio.loadmat(data_path + 'Indian_pines.mat')
    ...
    if Dataset == 'SV':
        SV = sio.loadmat(data_path + 'Salinas.mat')
    ...
    if Dataset == 'XA':
        ...
        data_hsi = SV['data_hsi']
        print(data_hsi.dtype)
        gt_hsi = gt_SV['roi_data']
        K = 8 
        TOTAL_SIZE = 3677110
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    ...
    Moran_Multi = []
    W1 = lat2W(*image.shape[:2], rook=False)
    Geary_Multi = []
    Variance = []
    for band in range(image.shape[2]):

        band_data = image_2d[:, band]
        moran = Moran(band_data, W, permutations=1999)
        Moran_Multi.append(moran)
        geary = Geary(band_data, W1, permutations=1999)
        Geary_Multi.append(geary)

        band_data0 = image[:, :, band]
        mask = (gt_hsi != 0)
        band_data_nonzero = band_data0 * mask
        variance = np.var(band_data_nonzero/10000.0)
        Variance.append(variance)
    
    p_values = [moran.p_sim for moran in Moran_Multi]
    z_scores = [moran.z_sim for moran in Moran_Multi]
    c_scores = [geary.z_sim for geary in Geary_Multi]

    fig, axs = plt.subplots(1, 3, figsize=(15, 6))
    
    I_values = [moran.I for moran in Moran_Multi]
    C_values = [geary.C for geary in Geary_Multi]
    IC_values= [i - c for i, c in zip(I_values, C_values)]
    V_values= [(1-v)*ic for ic, v in zip(IC_values, Variance)]
    thre=np.percentile(V_values, 20)
    print('thre: ', thre)

    filtered_I_values = [v if v > thre else None for v in V_values]
    axs[0].plot(I_values)
    axs[0].set_ylabel('Moran\'s I')
    axs[0].set_xlabel('Band')
    axs[1].plot(C_values, color= 'orange')
    axs[1].set_ylabel('Geary\'s C')
    axs[1].set_xlabel('Band')
    axs[2].plot(Variance, color= 'green')
    ...
    plt.figure()
    ###axs[0].plot(I_values, alpha=0.5)
    ###axs[0].plot(C_values, alpha=0.5)
    plt.plot(V_values, linewidth=2)
    plt.plot(filtered_I_values, color='red')
    plt.ylim(0,1)
    plt.legend()
    ...
    data = PCA(n_components=K).fit_transform(data)
    shapeor = np.array(shapeor)
    shapeor[-1] = K
    data = data.reshape(shapeor)
    
    return data, gt_hsi, TOTAL_SIZE, TRAIN_SIZE, VALIDATION_SPLIT
    '''
...
class S3KAIResNet(nn.Module):
    def __init__(self, band, classes, reduction):
        ...
        self.conv1x1 = nn.Conv3d(
            ...
            kernel_size=(1, 1, spec_dim),
            stride=(1, 1, 2),
            padding=0)
        self.conv3x3 = nn.Conv3d(
            in_channels=1,
            out_channels=PARAM_KERNEL_SIZE,
            kernel_size=(3, 3, spec_dim),
            stride=(1, 1, 2),
            padding=(1, 1, 0))
        ...
        self.res_net1 = Residual(
            PARAM_KERNEL_SIZE,
            PARAM_KERNEL_SIZE, (1, 1, spec_dim), (0, 0, spec_dim//2),
            start_block=True)
        self.res_net2 = Residual(PARAM_KERNEL_SIZE, PARAM_KERNEL_SIZE,
                                 (1, 1, spec_dim), (0, 0, spec_dim//2))
        ...
        kernel_3d = math.ceil((band - (spec_dim-1)) / 2)
        # print(kernel_3d)
...
def select(groundTruth):  #divide dataset into train and test datasets
    ...
    if Dataset == 'IN':
        ...
        amount = [
            ...
            876, 274, 69
        ]  #IP 20%, same amount of IP is better
        '''
        amount = [num_sample for _ in range(16)]
        '''
    ...
    if Dataset == 'UP':
        ...
        '''
        amount = [num_sample for _ in range(9)]
        '''
    num_sumple=80
    if Dataset == 'KSC':
        ...
        ]  #KSC, same amount of KSC is better
        '''
        amount = [num_sample for _ in range(13)]
        '''
    for i in range(m):
        indices = [
